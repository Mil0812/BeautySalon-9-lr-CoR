...
public class AssistanceTypeDao implements Dao<Integer, AssistanceType> {
	...
	private AssistanceTypeDao() {
	}

	private static class AssistanceTypeDaoHolder {

		public static final AssistanceTypeDao INSTANCE = new AssistanceTypeDao();
	}

	public static AssistanceTypeDao getInstance() {
		return AssistanceTypeDaoHolder.INSTANCE;
	}
	...
	public List<AssistanceType> getAll() {
		...
		try (Connection connection = ConnectionManager.getConnection();
			...
			while (resultSet.next()) {
				...
				assistanceTypes.add(AssistanceType.builder()
				    .id(id)
				    .name(name)
